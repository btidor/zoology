...
def disassemble(code: bytes) -> Program:
    ...
    while idx < len(code):
        ...
        if opref is None:
            ...
            program.jumps[idx - 1] = len(program.instructions)
        elif not hasattr(ops, opref.fullName):
            raise ValueError(f"unimplemented opcode: {opref.fullName}")

        program.instructions.append(Instruction(offset, opref.name, suffix, operand))
...
def printable_output(program: Program) -> str:
    output = ""
    for ins in program.instructions:
        ...
        if ins.operand is not None:
            operand = require_concrete(ins.operand).to_bytes(
                cast(int, ins.suffix), "big"
            )
            msg += "\t0x" + operand.hex()
        output += msg + "\n"
    return output
...
if __name__ == "__main__":
    ...
    output = printable_output(program)
    print(output, end="")
